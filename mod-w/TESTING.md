...
| Analytics consent banner title | `analytics-consent-title` |
| Analytics consent banner summary | `analytics-consent-summary` |
| Analytics consent accept control | `analytics-consent-accept` |
| Analytics consent reject control | `analytics-consent-reject` |
| Analytics consent Privacy Policy placeholder | `analytics-consent-privacy-placeholder` |
| Privacy / Cookie settings control (Footer) | `analytics-consent-settings` |

**Added (STEP-07):** no `analytics-consent-close` selector was added — STEP-07 §9 marked it optional, and Accept/Reject already provide the non-coercive close path the Step requires. Analytics event tracking (`section_navigation`, `contact_cta_click`, `outbound_profile_click`, `case_study_link_click`, `modw_repository_click`) is asserted in unit tests by providing a mock `GoogleAnalyticsService` and spying on `trackEvent`, not via new `data-testid` values — the CTAs/links being tracked already carry their existing STEP-01–06 test ids. Per Tech Lead review, the Privacy Policy mention moved from inline prose to its own clearly marked `analytics-consent-privacy-placeholder` element ("Privacy Policy — link coming soon") — STEP-07 §6 requires linking to, or reserving a *clear placeholder* for, the Privacy Policy/Datenschutzerklaerung, and plain unmarked prose didn't satisfy that; no working `href` exists yet because no URL is approved (`ARCHITECTURE.md` §15 open question).
...
| `e2e/portfolio.spec.ts` | Production shell renders, all mandatory launch Case Study titles visible, card field rendering, product/repository case-study link behavior, MOD-W runtime content loads (incl. roles/evidence), MOD-W CTAs visible, first-screen clarity, anchor nav through Case Studies/MOD-W/About/Contact, Contact paths and profile links, Footer attribution, mobile menu open/close, reduced-motion visibility of hero/work/MOD-W/contact content, keyboard tab order reaches nav/hero CTA/contact CTA; consent banner visible with no GA request before a decision, rejecting never loads GA, accepting loads the GA4 script with the configured measurement id (network request intercepted and aborted so nothing reaches Google from CI), a returning visitor with a valid accepted record is not re-prompted and GA still loads, withdrawing via the footer settings control stops further tracked requests, all at `/` | E2E |
...
| `src/app/portfolio/nav/nav.component.spec.ts` | Extended: Home is active by default with no scroll-spy wired up when `IntersectionObserver` is unavailable; the nav link for the most-visible observed section gets the `active` class and `aria-current`; tracks `section_navigation`/`contact_cta_click` on desktop, mobile-menu, and CTA clicks (not on scroll-spy changes) | Integration |
| `src/app/portfolio/case-study-card/case-study-card.component.spec.ts` | Extended: the classification badge carries the amber `proprietary` class only when `classification === 'proprietary'`; tracks `case_study_link_click` with `linkType` `product`/`repository`/`legacy` | Integration |
| `src/app/analytics/analytics-consent.service.spec.ts` | Starts `unknown` with no stored record, reads valid accepted/rejected records (via `isAnalyticsConsentRecord`), treats malformed/invalid-JSON/old-version records as `unknown`, persists accept/reject, supports withdrawal and re-acceptance, opens/closes settings, does not crash without `localStorage` | Unit |
| `src/app/analytics/google-analytics.service.spec.ts` | No script before consent, no-ops with an empty measurement id, injects the GA4 script exactly once after accept, sends exactly one initial page-view, no events before consent, approved events sent after consent, no further events after withdrawal, best-effort GA cookie cleanup attempted on withdrawal | Integration |
| `src/app/analytics/analytics-consent-banner/analytics-consent-banner.component.spec.ts` | Renders required test ids and accessible Accept/Reject controls when consent is `unknown`, reserves a clearly marked `analytics-consent-privacy-placeholder` (no working link yet, no URL approved), hides after accept/reject, persists the choice through `AnalyticsConsentService`, reopens when settings are opened | Integration |
| `src/app/portfolio/footer/footer.component.spec.ts` | Extended: renders the `analytics-consent-settings` control and opens analytics consent settings when clicked | Integration |
| `src/app/portfolio/hero-section/hero-section.component.spec.ts` | Extended: tracks `contact_cta_click` (`source: 'hero-work-cta'` / `'hero-modw-cta'`, `path: 'general'`) on the two hero CTAs | Integration |
| `src/app/portfolio/modw-section/modw-section.component.spec.ts` | Extended: tracks `modw_repository_click` on the repository CTA and `contact_cta_click` (`path: 'general'`) on the consulting CTA | Integration |
| `src/app/portfolio/contact-section/contact-section.component.spec.ts` | Extended: tracks `contact_cta_click` with `path: 'full-time'`/`'freelance'` per path CTA, and `outbound_profile_click` for LinkedIn/GitHub but not the plain email link | Integration |
| `src/app/app.spec.ts` | Extended: mounts `analytics-consent-banner` alongside the portfolio without disturbing it | Integration |

STEP-01 replaced the Angular starter placeholders (`src/app/app.spec.ts` starter assertions, `e2e/example.spec.ts`) with the tests above. STEP-02 added the nav/hero/engagement specs and extended `portfolio-page.component.spec.ts` and `e2e/portfolio.spec.ts`. STEP-03 extracted Case Studies rendering into dedicated `case-studies-section`/`case-study-card` components and specs, trimming the now-duplicated case-study detail assertions out of `portfolio-page.component.spec.ts`. STEP-04 extracted MOD-W rendering into a dedicated `modw-section` component and spec (same trim pattern), expanded the `ModwContent` contract, and added a production-content safety spec. STEP-05 replaced the `#about`/`#contact` placeholders with `about-section`/`contact-section`/`footer` components and specs, all static-content-driven (no runtime JSON, no loading/empty/error states needed). STEP-06 added the shared `SectionHeaderComponent`/`ChamferPanelComponent`/`RevealOnScrollDirective` (`src/app/shared/`), nav scroll-spy active-section state, decorative background layers, and the reduced-motion/keyboard E2E checks, without changing any existing `data-testid` value. STEP-07 added the `src/app/analytics/` feature boundary (`AnalyticsConsentService`, `GoogleAnalyticsService`, `AnalyticsConsentBannerComponent`) and wired `trackEvent` calls into nav, hero, MOD-W, Contact, and Case Study Card, all gated behind accepted consent; no existing runtime JSON contract or `data-testid` changed.
